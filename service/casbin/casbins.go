...
type InterfaceCasbin interface {
	AddPolicy(policy []*models.CasbinPolicy) error
	DelPolicy(policy []*models.CasbinPolicy) error
	ListPolicy(keyWord string, limit, page int) *models.CasbinPolicyList
}
...
func (c *casbinInfo) AddPolicy(policy []*models.CasbinPolicy) error {
	if len(policy) > 0 {
...
func (c *casbinInfo) DelPolicy(policy []*models.CasbinPolicy) error {
	if len(policy) > 0 {
		for _, item := range policy {
...
func (c *casbinInfo) ListPolicy(keyWord string, limit, page int) *models.CasbinPolicyList {
	var (
		policy models.CasbinPolicy
		total  int
	)
	policys := make([]models.CasbinPolicy, 0)
	...
	// 自定义处理分页
	if limit <= 0 || page <= 0 {
		return &models.CasbinPolicyList{
			Items: policys,
			Total: total,
		}
	}
